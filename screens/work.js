...
class Work extends React.Component {
  ...
  async loadData() {
    try {
      ...
      // Filter tasks specific to Work
      const workTasks = tasks.filter(task => task.page === 'Work');
  ...
  async deleteTask(index) {
    try {
      ...
      // Update state with filtered tasks for Work page
      const workTasks = tasks.filter(task => task.page === 'Work');
      this.setState({ tasks: workTasks });
    } catch (error) {
                ...
                <Text style={styles.descriptiontekst}>
                  {expandedDescriptions[index] 
                    ...
                    : task.description.length > 12 
                      ? task.description.substring(0, 12).trim() + '...'
                      : task.description
                  }
                </Text>
              </TouchableOpacity>
