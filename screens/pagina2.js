...
class Pagina2 extends React.Component {
  ...
  async loadData() {
    try {
      const tasksString = await AsyncStorage.getItem('tasksPagina2'); // Use a unique key for Pagina2
      const tasks = JSON.parse(tasksString) || [];

      // Filter tasks specific to Pagina2
      const pagina2Tasks = tasks.filter(task => task.page === 'Pagina2');
      this.setState({ tasks: pagina2Tasks });
    } catch (error) {
  ...
  async loadData() {
    try {
      const tasksString = await AsyncStorage.getItem('tasks'); // Use the common key for tasks
      const tasks = JSON.parse(tasksString) || [];
  
      // Filter tasks specific to the current page
      const pageName = this.props.route.name; // Assuming you're using React Navigation
      const pageTasks = tasks.filter(task => task.page === pageName);
      this.setState({ tasks: pageTasks });
    } catch (error) {
      console.error('Error loading data:', error);
    }
  }
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.boxes}>
          ...
          <TouchableOpacity 
            style={styles.footer}
            onPress={() => this.handleBoxPress('Make2')}
          >  
